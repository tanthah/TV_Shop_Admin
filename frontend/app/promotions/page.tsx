...
export default function PromotionsPage() {
  ...
  const createCoupon = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`${baseURL}/coupons`, form, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
  ...
  const setActive = async (id: string, value: boolean) => {
    try {
      await axios.patch(`${baseURL}/coupons/${id}/active?value=${value}`, {}, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
      fetchCoupons();
    } catch (err: any) {
  ...
  const removeCoupon = async (id: string) => {
    if (!confirm('Bạn có chắc muốn xoá mã khuyến mãi này?')) return;
    try {
      await axios.delete(`${baseURL}/coupons/${id}`, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
      fetchCoupons();
    } catch (err: any) {
