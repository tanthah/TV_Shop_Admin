...
export default function RegisterPage() {
  ...
  const sendOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${baseURL}/api/register/send-register-otp`, { email });
      if (res.data?.success) {
        setStep(2);
      } else {
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      const res = await axios.post(`${baseURL}/api/register/verify-register-otp`, { email, otp });
      if (res.data?.success) {
        setStep(3);
      } else {
  ...
  const completeRegister = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const res = await axios.post(`${baseURL}/api/register/complete-register`, form, {
