...
export default function ProductsPage() {
  ...
  const fetchProducts = async () => {
    ...
    setError('');
    try {
      const res = await axios.get(`${baseURL}/api/products/admin/list`, {
  ...
  const handleImageSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length + images.length + existingImages.length > 20) {
      setError('Tối đa 20 hình ảnh');
      return;
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (editingProduct) {
        ...
        await axios.patch(`${baseURL}/api/products/${editingProduct._id}`, fd, {
          headers: {
        ...
        await axios.post(`${baseURL}/api/products`, fd, {
          headers: {
            'Content-Type': 'multipart/form-data',
            Authorization: `Bearer ${Cookies.get('admin_token') || ''}`,
          },
  ...
  const toggleStatus = async (product: Product) => {
    ...
    try {
      await axios.patch(`${baseURL}/api/products/${product._id}/status`, {}, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
  ...
  const removeProduct = async (id: string, name: string) => {
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('Lỗi', err.response?.data?.message || 'Không thể xoá sản phẩm', 'error');
      }
    }
  };
              ...
              <tr key={p._id}>
                ...
                <td>
                  <div className="admin-actions">
                    ...
                    <button
                      className="admin-button danger icon-only sm"
                  ...
                  <div className="form-group full-width">
                    ...
                    <div className="admin-file-input">
                      <input
                        ...
                        disabled={images.length + existingImages.length >= 20}
                      />
                      <div className="admin-file-input-label">
                        📷 {images.length + existingImages.length > 0
                          ? `Đã chọn ${images.length + existingImages.length} ảnh (tối đa 20)`
                          : 'Chọn ảnh sản phẩm'}
                      </div>
                    </div>
                  </div>
