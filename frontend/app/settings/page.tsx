  ...
  const save = async () => {
    ...
    try {
      const payload = {
        ...
        socialLinks: data.general?.socialLinks,
      ...
      const res = await axios.patch(`${baseURL}/api/settings`, payload, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
      setData(res.data?.data);
            ...
            <div className="form-grid">
              ...
              <div className="form-group">
                ...
                <input
                  id="address"
                  className="admin-input"
                  placeholder="VD: 1 Võ Văn Ngân, TP.HCM"
                ...
                <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 16 }}>
                  <div>
                    ...
                    <input
                      ...
                      value={data.general?.socialLinks?.facebook || ''}
                      onChange={(e) => updateField('general', 'socialLinks', { ...data.general.socialLinks, facebook: e.target.value })}
                    />
                  </div>
                  <div>
                    ...
                    <input
                      ...
                      value={data.general?.socialLinks?.instagram || ''}
                      onChange={(e) => updateField('general', 'socialLinks', { ...data.general.socialLinks, instagram: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="form-label small text-muted">YouTube</label>
                    <input
                      className="admin-input"
                      placeholder="URL YouTube"
                      value={data.general?.socialLinks?.youtube || ''}
                      onChange={(e) => updateField('general', 'socialLinks', { ...data.general.socialLinks, youtube: e.target.value })}
                    />
                  </div>
                  <div>
                    <label className="form-label small text-muted">TikTok</label>
                    <input
                      className="admin-input"
                      placeholder="URL TikTok"
                      value={data.general?.socialLinks?.tiktok || ''}
                      onChange={(e) => updateField('general', 'socialLinks', { ...data.general.socialLinks, tiktok: e.target.value })}
                    />
                  </div>
                </div>
              </div>
