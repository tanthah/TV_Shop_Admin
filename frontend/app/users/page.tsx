...
export default function UsersPage() {
  ...
  const fetchUsers = async () => {
    ...
    setError('');
    try {
      const res = await axios.get(`${baseURL}/api/users/admin/list`, {
  ...
  const updateUser = async (id: string, payload: any) => {
    try {
      await axios.patch(`${baseURL}/api/users/${id}`, payload, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
      fetchUsers();
    } catch (err: any) {
  ...
  const deleteUser = async (id: string, name: string) => {
    const result = await Swal.fire({
      ...
      text: "Hành động này sẽ XÓA VĨNH VIỄN tài khoản và không thể khôi phục!",
      icon: 'warning',
    ...
    if (result.isConfirmed) {
      try {
        ...
        Swal.fire('Lỗi', err.response?.data?.message || 'Không thể xoá người dùng', 'error');
      }
    }
  };
  ...
  const uploadAvatar = async (id: string, file: File | null) => {
    ...
    try {
      ...
      await axios.post(`${baseURL}/api/users/${id}/avatar`, fd, {
        headers: {
  ...
  return (
    <div className="admin-container">
                ...
                <td>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <label className="admin-file-input" style={{ width: 80 }}>
                      ...
                      />
                      <span style={{
                        fontSize: 11, color: 'var(--primary-500)', cursor: 'pointer', textDecoration: 'underline'
                      }}>
                        Đổi ảnh
                      </span>
                    </label>
                ...
                <td>
                  <div className="admin-actions">
                    <Link
                      ...
                      style={{ textDecoration: 'none', display: 'flex', alignItems: 'center', height: 28, fontSize: 13 }}
                    >
                      Xem
                    </Link>
                    <button
              ...
              <tr>
                <td colSpan={7}>
                  <div className="admin-empty">
                    ...
                    <div className="admin-empty-text">Chưa có người dùng nào</div>
                  </div>
                </td>
              </tr>
            )}
      ...
      <div className="admin-pagination">
        ...
        <span className="page-info">Trang {page} / {totalPages || 1} • Tổng {total} người dùng</span>
        <button className="admin-button sm" disabled={(page * limit) >= total} onClick={() => setPage(page + 1)}>Sau →</button>
      </div>
    </div>
  );
}
