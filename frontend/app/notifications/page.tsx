...
export default function NotificationsPage() {
  ...
  const fetchNotifications = async () => {
    ...
    setError('');
    try {
      const res = await axios.get(`${baseURL}/api/notifications/admin/list`, {
  ...
  const createNotification = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      await axios.post(`${baseURL}/api/notifications`, form, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
  ...
  const setRead = async (id: string, value: boolean) => {
    try {
      await axios.patch(`${baseURL}/api/notifications/${id}/read?value=${value}`, {}, {
        headers: { Authorization: `Bearer ${Cookies.get('admin_token') || ''}` },
      });
      fetchNotifications();
    } catch (err: any) {
